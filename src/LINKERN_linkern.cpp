...
AdditionalLogData* LOGDATA;
#define PRINT_INPUT_DATA false
...
CC_PTRWORLD_LEAKS_ROUTINE(intptr, intptr_check_leaks, this_2, int)
...
int CClinkern_tour (int ncount, compass_data *dat, int ecount,
        ...
        int *outcycle, double *val,
        int silent, double time_bound, double length_bound,
        char *saveit_name, int kicktype, CCrandstate *rstate, AdditionalLogData* logData)
{
    LOGDATA = logData;
    if (PRINT_INPUT_DATA){
    	printf(">>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
    	...
    	//for (int i=0; i < dat->n; i++){
    		//printf("    (%d, %d, %d)", dat->x[i], dat->y[i], dat->z[i]);
    	//}
    	//printf("\n");
    	...
    	printf("    lenspace: ");
    	//for (int j = 0; j < dat->n; j++){
    		//printf("%d ", dat->lenspace[j]);
    	//}
    	//printf("\n");
    	printf("    degree: ");
    	//for (int j = 0; j < dat->n; j++){
    		//printf("%d ", dat->degree[j]);
    	//}
    	printf("\n");


    	printf("    norm: %d\n", dat->norm);
    	printf("    dsjrand_param: %d\n", dat->dsjrand_param);
    	printf("    default_len: %d\n", dat->default_len);
    	printf("    sparse_ecount: %d\n", dat->sparse_ecount);
    	printf("    gridsize: %f\n", dat->gridsize);
    	printf("    dsjrand_factor: %f\n", dat->dsjrand_factor);

    	printf("    --rhdat: \n");
    	printf("      rhdat.dist_00: %d\n", dat->rhdat.dist_00);
    	printf("      rhdat.dist_01: %d\n", dat->rhdat.dist_01);
    	printf("      rhdat.dist_02: %d\n", dat->rhdat.dist_02);
    	printf("      rhdat.dist_12: %d\n", dat->rhdat.dist_12);
    	printf("      rhdat.dist_22: %d\n", dat->rhdat.dist_22);

    	printf("      rhdat.p: %f\n", dat->rhdat.p);
    	printf("      rhdat.rhlength: %d\n", dat->rhdat.rhlength);
    	printf("      rhdat.space: %p\n", dat->rhdat.space);

    	//printf("      rhdat.vectors[0]: %p\n", dat->rhdat.vectors[0]);
    	...
    	printf("    --userdat:\n");
    	//printf("      userdat.x[0]: %f\n", dat->userdat.x[0]);
    	//printf("      userdat.y[0]: %f\n", dat->userdat.y[0]);
    	...
    	printf("    ndepot: %d\n", dat->ndepot);
    	printf("    orig_ncount: %d\n", dat->ndepot);
    	printf("    depotcost: >");
    	for (int i=0; i < dat->orig_ncount; i++){
    		printf("    %d", dat->depotcost[i]);
    	}
    	printf("<\n");
    	printf("    orig_names:>");
    	for (int i=0; i < dat->orig_ncount; i++){
    		printf("    %d", dat->orig_names[i]);
    	}
    	printf("<\n");
    ...
    //{  int    (*edgelen) ( int i, int j, struct compass_data *data);
    	...
    	printf("silent: %d\n", silent);
    	printf("time_bound: %f\n", time_bound);
    	printf("length_bound: %f\n", length_bound);
    	printf("--saveit_name: %s\n", saveit_name);
    	printf("kicktype: %d\n", kicktype);
    	printf("--rstate: %d\n", 0);
    	//printf("  a: %d\n", rstate->a);
    	//printf("  b: %d\n", rstate->b);
    	//printf("  arr: ");
    	//for (int i=0; i < 55; i++){
    		//printf("%d ", rstate->arr[i]);
    	//}
    	//printf("\n");

    	printf("<<<<<<<<<<<<<<<<<<<<<<<<<<\n");
    	waitForInput("input printed", true);
    }
    ...
    *val = cycle_length (ncount, tcyc, &D);
    ...
    if (silent == 0) {
        ...
        printf ("Lin-Kernighan Running Time (ToDo: insert value here): %.2f\n", 0.0);
        fflush (stdout);
    }
        ...
        CCrandstate *rstate)
{
    ...
    if (silent == 0) {
        if (quitcount > 0) {
            printf ("%4d Steps   Best (To-Do: insert value): %.0f   %.2f seconds\n", round, best,
                                0.0);
        } else {
            printf ("LK Cycle: %.0f\n", best);
        }
...
static int step (graph *G, distobj *D, adddel *E, aqueue *Q, CClk_flipper *F,
        ...
        CCptrworld *edgelook_world)
{
    int val, this_2, newlast, hit = 0, oldG = gain;
    ...
    for (e = list; e; e = e->next) {
        ...
        if (e->mm) {
            ...
            FLIP (this_2, newfirst, first, last, fstack, F);

            if (level < MAXDEPTH) {
                markedge_add (first, this_2, E);
                markedge_del (this_2, newfirst, E);
                hit += step (G, D, E, Q, F, level + 1, gain, Gstar, newfirst,
                             last, fstack, intptr_world, edgelook_world);
                unmarkedge_add (first, this_2, E);
                unmarkedge_del (this_2, newfirst, E);
            }

            if (!hit) {
                UNFLIP (this_2, newfirst, first, last, fstack, F);
            } else {
                MARK (this_2, Q, F, D, G, intptr_world);
...
#endif
        {
            ...
            FLIP (first, last, newlast, this_2, fstack, F);

            if (level < MAXDEPTH) {
                markedge_add (last, this_2, E);
                markedge_del (this_2, newlast, E);
                hit += step (G, D, E, Q, F, level + 1, gain, Gstar, first,
                             newlast, fstack, intptr_world, edgelook_world);
                unmarkedge_add (last, this_2, E);
                unmarkedge_del (this_2, newlast, E);
            }

            if (!hit) {
                UNFLIP (first, last, newlast, this_2, fstack, F);
            } else {
                MARK (this_2, Q, F, D, G, intptr_world);
        ...
        flipstack *win, flipstack *fstack, CCptrworld *intptr_world)
{
    ...
    for (i = 0; goodlist[last][i].weight < cutoff; i++) {
        this_2 = goodlist[last][i].other;
        if (!is_it_deleted (last, this_2, E) && this_2 != first &&
                                              this_2 != lastnext) {
            prev = CClinkern_flipper_prev (F, this_2);
            if (!is_it_added (this_2, prev, E)) {
                val = goodlist[last][i].weight - Edgelen (this_2, prev, D);
                if (val < winner.diff) {
                    winner.diff = val;
                    winner.other = this_2;
                    winner.over = prev;
    ...
    if (winner.diff < BIGINT) {
        ...
        *Gstar = gain - Edgelen (newlast, first, D);
        ...
        FLIP (first, last, newlast, this_2, fstack, F);
        kickturn (this_2, Q, D, G, F, intptr_world);
       ...
       int first, int last, int gain, int level, CCptrworld *edgelook_world)
{
...
#if defined(MAK_MORTON) && defined(FULL_MAK_MORTON)
    {
        ...
        for (i = 0; goodlist[first][i].weight < gain; i++) {
#endif
            this_2 = goodlist[first][i].other;
            if (!is_it_deleted (first, this_2, E) && this_2 != last &&
                                                   this_2 != firstprev) {
                next = CClinkern_flipper_next (F, this_2);
                if (!is_it_added (this_2, next, E)) {
                    val = goodlist[first][i].weight - Edgelen (this_2, next, D);
                    if (val < value[0]) {
                        for (k = 0; value[k+1] > val; k++) {
                            value[k] = value[k+1];
                            other[k] = other[k+1];
        ...
        if (!is_it_deleted (last, this_2, E) && this_2 != first &&
            ...
            if (!is_it_added (this_2, prev, E)) {
                ...
                if (!is_it_added (this_2, next, E) &&
                    ...
                    val += (Edgelen (next, prev, D) - Edgelen (this_2, next, D));
                    if (val < winner->diff) {
                        winner->diff = val;
                        winner->other = this_2;
                        winner->over = prev;
        ...
        int gain, int t1, int t2, CCptrworld *edgelook_world)
{
    ...
    for (i = 0; goodlist[t2][i].weight < gain; i++) {
#endif
        this_2 = goodlist[t2][i].other;
        if (this_2 != t1) {
            next = CClinkern_flipper_next (F, this_2);
            val = goodlist[t2][i].weight - Edgelen (this_2, next, D);
            if (val < value[0]) {
                for (k = 0; value[k+1] > val; k++) {
                    value[k] = value[k+1];
                    other[k] = other[k+1];
...
//      printf("val: %f\n",val);
    LOGDATA->numberOfCalculatedPaths_++;
    return val;
}
...
static int dist (int i, int j, distobj *D)   /* As in Bentley's kdtree paper */
{
    ...
    LOGDATA->numberOfShortestPathCalls_++;
